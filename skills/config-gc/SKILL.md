---
name: hkx-config-gc
description: Garbage collection for OMP configuration. Periodically scans .omp/ and ~/.omp/ for redundant, stale, orphaned, or low-value items, then walks the user through a confirm-each-deletion cleanup. Use when the user says "clean up my config", "config GC", "too many skills", "audit my setup", or asks for a periodic config review.
origin: HKX-converted-for-OMP
---

# HKX Config GC — Garbage Collection for OMP Setups
...
- The user asks to clean up, audit, or slim down their OMP configuration
...
|---|---------|------|--------------------------------|
| 1 | Skills | `.omp/skills/*/` or `~/.omp/skills/*/` | Heavily overlapping names; domain mismatch with the user's actual work; broken or empty SKILL.md |
| 2 | Memory | `.omp/memory/*.md` + its index | Multiple index entries for one topic; contents contradicting newer entries; dates that have passed; orphan files missing from the index; sub-100-word fragments that should merge |
| 3 | Hooks | `.omp/hooks/` + settings | Scripts present on disk but referenced by no hook config; old versions superseded by rewrites |
...
| 6 | Commands | `.omp/commands/*/` | Commands that duplicate built-in OMP behavior; commands with broken skill/tool references |
| 7 | Rules | `.omp/rules/` | Rules that contradict project AGENTS.md; rules with stale references |
| 8 | Runtime caches | `.omp/cache/`, `.omp/logs/` | Sort by size and mtime; propose items >30 days old and large |
...
- **Touching anything outside `.omp/` or the project's `.omp/`.** Config GC never wanders into source trees.
